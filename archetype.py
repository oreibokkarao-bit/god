...
TIER 1 (expectancy / edge):
- Robust normalized composite score (logistic-reg-style) using per-symbol features:
  vol_sum_5m, ret_15m, cvd_slope_5m, ob_imb, oi_chg, rv_15m, range_15m.
- 24 archetypes, all running in parallel (non-cancelling), adaptive via z-scores & regimes.
- Multi-timeframe alignment (approx 1h/4h trend from real trades).
...
- TP1/TP2/TP3 & SL derived from these zones, not % guesses, with RR + big-move guards.
- SQLite trade life log: OPEN -> TP/SL verdict with RR filter.
- Tier-1 leverage governor (vol/liquidity/RR aware) + leverage suggestion column.

TIER 2 (architecture / robustness / observability):
- Async I/O (aiohttp + websockets) for Binance Futures streams.
- ThreadPoolExecutor for CPU-heavy feature + scoring work (hybrid concurrency).
- Token-bucket rate limiter + 418/429-aware safe_request() wrapper for REST calls.
- Optional Prometheus metrics (if prometheus_client installed):
  - trade counts, table build latency, WS message volume.
- Structured JSON logging for trade lifecycle + key events.

Honesty / data sources:
- All signals (price, volume, OI, orderbook, trades) come directly from Binance Futures public endpoints.
- CVD is computed from the raw @trade stream using Binance's taker/maker flag
  (isBuyerMaker → taker side = not isBuyerMaker).
- Pivots and magnetic levels are built entirely from real traded prices in the current window
...
METRIC_TRADE_OPENED = Counter(
    "archetype_trades_opened_total",
    "Number of trades opened by Archetype engine",
)
METRIC_TRADE_CLOSED = Counter(
    "archetype_trades_closed_total",
    "Number of trades closed by Archetype engine",
    ["verdict"],
)
METRIC_TABLE_BUILD_SECONDS = Histogram(
    "archetype_table_build_seconds",
    "Time to build setups table (seconds)",
)
METRIC_WS_MESSAGES = Counter(
    "archetype_ws_messages_total",
...
# HTF windows for trend detection (based only on trades we see)
TREND_1H_SEC = 60 * 60
TREND_4H_SEC = 4 * 60 * 60

MIN_NOTION_USDT = 100_000  # min 5m notional
MIN_PRICE = 0.0005
...
GLOBAL_RAW_SCORE_GATE = 2.0   # hard gate on raw composite
ALPHA_THRESHOLD = -0.1         # soft gate on EWMA alpha

# Risk management
MIN_RR = 1.5   # minimum RR to surface & trigger

# Big-move requirement: we don't want chump 1–2% wiggles
MIN_PUMP_PCT = 0.03  # require at least ~5% distance to mid/upper targets

# Volatility-aware & sanity guards for SL / RR
MIN_SL_PCT = 0.003      # base 0.3% minimum stop distance from entry
MAX_SL_PCT = 0.02       # base 2% maximum stop distance
...
def suggest_leverage_for_symbol(
    ...
    # very borderline RR trades don't get huge leverage
    if rr < MIN_RR + 0.5:
...
class SymbolState:
    ...
    # symbol-level baselines for adaptive RVOL / volatility
    vol_5m_ema: float = 0.0
    rv_15m_ema: float = 0.0
    ...
    def compute_features(self, now_ts: float) -> Dict[str, float]:
        ...
        # update symbol-level EMAs & vol regime
        self.vol_5m_ema = ema(self.vol_5m_ema, feats["vol_sum_5m"], alpha=0.2)
        self.rv_15m_ema = ema(self.rv_15m_ema, feats["rv_15m"], alpha=0.2)

        if self.rv_15m_ema <= 0:
            self.vol_regime = "normal"
        else:
            if feats["rv_15m"] < 0.5 * self.rv_15m_ema:
                self.vol_regime = "calm"
            elif feats["rv_15m"] > 1.5 * self.rv_15m_ema:
...
class TargetCalculator:
    ...
    def compute_targets(
        ...
        def _apply_risk_guards(entry, sl, tp1, tp2, tp3, direction):
            ...
            # Big-move filter: we want at least ~5% potential
            if direction == "long":
                pump_pct = safe_div(tp1 - entry, entry)
            else:
                pump_pct = safe_div(entry - tp1, entry)
...
async def _safe_read_json(resp: aiohttp.ClientResponse):
    try:
        return await resp.json()
    except Exception:
        text = await resp.text()
        raise RuntimeError(f"Failed to parse JSON: {text[:200]}...")
...
class BinanceFuturesConnector:
        ...
        for attempt in range(max_retries):
            ...
            try:
                async with self.session.request(method, url, params=params, timeout=10) as resp:
                    ...
                    return await _safe_read_json(resp)
    ...
    async def _consume_trade_ws(self, ws: aiohttp.ClientWebSocketResponse):
        async for msg in ws:
            if msg.type == aiohttp.WSMsgType.TEXT:
                METRIC_WS_MESSAGES.labels(stream="trade").inc()
    ...
    async def _consume_depth_ws(self, ws: aiohttp.ClientWebSocketResponse):
        async for msg in ws:
            if msg.type == aiohttp.WSMsgType.TEXT:
                METRIC_WS_MESSAGES.labels(stream="depth").inc()
                data = json.loads(msg.data)
                payload = data.get("data")
                if not payload:
                    continue
...
# ======================================================================
# Feature compute helper for ThreadPoolExecutor
# ======================================================================

def compute_features_for_symbol(args):
    sym, st, now_ts = args
    return sym, st.compute_features(now_ts)
...
class BreakoutArchetypeApp:
    ...
    async def start(self):
        ...
        if METRICS_ENABLED:
            ...
            logger.info("Prometheus metrics exposed on :8008")
        ...
        log_event("universe_prepared", symbols=len(symbols))
    ...
    def _update_trade_db(self, trade: Trade):
        ...
        METRIC_TRADE_CLOSED.labels(verdict=trade.verdict or "-").inc()
        log_event(
            "trade_closed",
            id=trade.db_id,
            symbol=trade.symbol,
    ...
    def _build_setups_table(self) -> Tuple[Panel, List[str]]:
        ...
        # Tier-2: parallel feature compute via ThreadPoolExecutor
        items = list(self.states.items())
        if items:
            args = [(sym, st, now_ts) for sym, st in items]
            for sym, feats in self.executor.map(compute_features_for_symbol, args):
                feats_by_symbol[sym] = feats

        # Build scalers
        for sym, feats in feats_by_symbol.items():
        ...
        for sym, st in self.states.items():
            feats = feats_by_symbol.get(sym)
            if feats is None:
                # never saw any trades yet
                continue
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            table.add_row(
                ...
                f"{lev:.0f}x" if lev is not None else "-",
            )

        subtitle_text = (
            f"Universe: Binance USDT Perps • Raw score ≥ {GLOBAL_RAW_SCORE_GATE:.2f} "
            f"| RR ≥ {MIN_RR:.1f} | 24 archetypes + Magnetic Zones | Leverage governor"
        )
        ...
        duration = now_s() - start_build
        METRIC_TABLE_BUILD_SECONDS.observe(duration)
