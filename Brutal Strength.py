#!/usr/bin/env python3
"""
Brutal Strength Radar – Full Binance Futures long scanner
...
- Renders a stable Rich layout with 3 tables:
    1. Top Strong Longs (signals)
    2. Live Long Trades (virtual, dry-run)
    3. Closed Long Trades & Verdicts
- Persists signals and trades to SQLite:
    brutal_strength_trades.sqlite (in the SAME folder as this script)
...
class ScannerConfig:
    ...
    db_path: Path = field(
        default_factory=lambda: Path(__file__).with_name("brutal_strength_trades.sqlite")
    )
...
# ----------------------------------------------------------------------------
# Feature engineering & scoring – LONG bias
# ----------------------------------------------------------------------------
...
def compute_strength_score(
    ...
    """
    Brutal Strength score:
    - Reward: coins up on the day, off the lows, with growing OI.
    - Penalize: already exhausted / right at the 24h high with crazy funding.
    """
    reasons: List[str] = []

    # 1. 24h up-move (momentum)
    up_score = max(price_change_pct, 0.0)
    if up_score > 3:
        reasons.append(f"24h +{price_change_pct:.1f}% (rising momentum)")

    # 2. Distance above 24h low (off the floor)
    off_floor = max(dist_from_low_pct - 3.0, 0.0)
    if dist_from_low_pct > 3:
        reasons.append(f"{dist_from_low_pct:.1f}% above 24h low (off the floor)")

    # 3. Not too far from the high (avoid deep pullbacks)
    high_penalty = max(dist_from_high_pct - 40.0, 0.0)
    if dist_from_high_pct < 20:
        reasons.append(f"{dist_from_high_pct:.1f}% below 24h high (in breakout zone)")

    # 4. Funding – positive but not insane
    funding_bps = funding_rate * 10_000.0
    funding_score = 0.0
    if 0 < funding_bps <= 30:
        funding_score += funding_bps * 0.1
        reasons.append(f"Funding +{funding_bps:.1f} bps (healthy long interest)")
    elif funding_bps > 30:
        funding_score -= (funding_bps - 30.0) * 0.2
        reasons.append(f"Funding +{funding_bps:.1f} bps (crowded, late longs)")

    # 5. OI dynamics – up with price is ideal
    oi_score = 0.0
    if oi_change_pct is not None:
        if oi_change_pct > 5:
            oi_score += 5.0
            reasons.append(f"OI +{oi_change_pct:.1f}% (fresh long build)")
        elif oi_change_pct < -5:
            oi_score -= 3.0
            reasons.append(f"OI −{abs(oi_change_pct):.1f}% (longs exiting)")
...
def compute_tp_sl(
    ...
    """
    Advanced TP/SL engine (LONG bias), with wick protection:

    - SL anchored to 24h low + volatility factor.
    - PLUS an ATR-like floor based on 24h range so SL is never too tight.
    - PLUS extra widening for very high scores (we expect trend, don't get wicked).
    - TP1/TP2/TP3 based on 24h range + strength score buckets.
    """
    ...
    unit_pct = range_pct / 4.0  # one "unit" of move

    # --- 2) Score-based aggression multipliers for TPs ---
    if score < 20:
        # softer, mean-reversion style long
        m1, m2, m3 = 0.6, 1.0, 1.3
    elif score < 35:
        # normal trend
        m1, m2, m3 = 0.9, 1.4, 1.9
    else:
        # brutal strength, aim deeper
        m1, m2, m3 = 1.1, 1.7, 2.3

    tp1_pct = (unit_pct * m1) / 100.0
    tp2_pct = (unit_pct * m2) / 100.0
    tp3_pct = (unit_pct * m3) / 100.0

    # --- 3) Reconstruct 24h low and base SL from low (structure + vol) ---
    if dist_from_low_pct > 0.01:
        # dist_from_low_pct ≈ (last - low)/last * 100 → low = last * (1 - d/100)
        low = entry * (1.0 - dist_from_low_pct / 100.0)
    else:
        # fallback: synthetic low slightly below entry, scaled by volatility
        synthetic_drop = max(0.005, min(0.02, range_pct / 200.0))
        low = entry * (1.0 - synthetic_drop)
    ...
    sl_from_low = low * (1.0 - cfg.sl_buffer_pct * vol_factor)
    ...
    if score > 50:
        score_extra_pct = 0.5   # widen stop a bit further
    elif score > 35:
        score_extra_pct = 0.35
    else:
        score_extra_pct = 0.0

    # Combine all SL protections (for LONG, SL must be BELOW entry)
    sl = min(sl_from_low, sl_from_entry_floor)
    sl *= (1.0 - score_extra_pct / 100.0)

    # Guard against negative/too small SL
    sl_floor = entry * 0.02  # don't let SL go below a 98% dump (just a sanity clamp)
    sl = max(sl, sl_floor)

    # --- 6) Final TP prices for LONGS ---
...
# ----------------------------------------------------------------------------
# Rich UI helpers – non-jumping layout
# ----------------------------------------------------------------------------


def style_pct(val: Optional[float]) -> str:
    """For LONGS: green positive, red negative, white neutral 0."""
...
def format_oi(val: Optional[float]) -> str:
    """Format raw OI nicely (contracts) into K/M/B with no colors."""
    if val is None or math.isnan(val):
        return "-"
    v = float(val)
    if v >= 1e9:
...
def build_signals_table(features: List[StrengthFeatures]) -> Table:
    table = Table(
        title="🚀 Brutal Strength – Top Longs (Binance USDT-Perps)",
    ...
    for idx in range(rows_to_show):
        if idx < len(features):
            ...
            up_from_low_str = style_small_pct(f.dist_from_low_pct)
            down_from_high_str = style_small_pct(-f.dist_from_high_pct)  # show as negative
            ...
            # Raw OI (may be None)
            oi_raw_str = format_oi(f.oi_raw)

            # OI change – if we have raw OI but no change yet, show 0.0%
            if f.oi_change_pct is None:
                if f.oi_raw is not None:
                    oi_delta_str = style_small_pct(0.0)
                else:
                    oi_delta_str = "-"
            else:
                oi_delta_str = style_small_pct(f.oi_change_pct)
        ...
        if idx < len(live_trades):
            ...
            if last and entry:
                if side.upper() == "LONG":
                    ...
                    pl_pct = (entry - last) / entry * 100.0  # shouldn't happen here
...
def build_closed_trades_table(closed_trades: List[Tuple]) -> Table:
    table = Table(
        title="✅ Closed Long Trades & Verdicts",
        box=box.MINIMAL_HEAVY_HEAD,
        expand=True,
        show_lines=False,
    )
...
# ----------------------------------------------------------------------------
# Trade engine (LONG, paper-only)
# ----------------------------------------------------------------------------


async def manage_trades(
    store: TradeStore,
    top_signals: List[StrengthFeatures],
    price_lookup: Dict[str, float],
) -> None:
    """
    Very simple paper-trade engine (LONG bias):
    - If a symbol is in the top 3 signals and has no open trade → open LONG.
    ...
    for f in top_signals[:3]:
        ...
        sl, tp1, tp2, tp3 = compute_tp_sl(
            entry=entry,
            dist_from_low_pct=f.dist_from_low_pct,
        ...
        with Live(console=console, screen=True, refresh_per_second=4) as live:
            try:
                while not stop_event.is_set():
                    ...
                    features: List[StrengthFeatures] = []
                    price_lookup: Dict[str, float] = {}
                    ...
                    for sym in symbols:
                        ...
                        dist_from_low_pct = (last_price - low_price) / last_price * 100.0
                        dist_from_high_pct = (1.0 - last_price / high_price) * 100.0

                        price_lookup[sym] = last_price

                        # OI change & raw from Bybit cache
                    ...
                    # Rank by descending strength score
                    ...
                    layout = Group(
                        Panel(signals_table, title="Brutal Strength Radar", border_style="bright_blue"),
                        Panel(live_table, title="Long Trade Monitor (Dry-Run)", border_style="green"),
                        Panel(closed_table, title="Long Trade Verdicts", border_style="magenta"),
                    )
                    live.update(layout)
